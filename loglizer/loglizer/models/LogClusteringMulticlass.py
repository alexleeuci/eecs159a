...
class LogClusteringMulticlass(object):
    ...
    def predict(self, X):
        ...
        for i in range(X.shape[0]):
            ...
            #print("debug: mindist and minindex", min_dist,min_index)
    ...
    def _offline_clustering(self, X):
        ...
        if self.linkageMode==0:
            Z = linkage(p_dist, 'single')
        elif self.linkageMode==1:
            Z = linkage(p_dist, 'complete')
        elif self.linkageMode==2:
            Z = linkage(p_dist, 'average')
        elif self.linkageMode==3:
            Z = linkage(p_dist, 'weighted')
